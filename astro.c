...
PG_FUNCTION_INFO_V1(scircle_deg);

float8 deg2rad(float8 deg) { return deg * 0.017453292519943295; }
float8 rad2deg(float8 rad) { return rad * 57.29577951308232; }
...
{
  float8 ra_deg = PG_GETARG_FLOAT8(0);
  float8 dec_deg = PG_GETARG_FLOAT8(1);
...
Datum scircle_deg(PG_FUNCTION_ARGS)
{
  float8 ra_deg = PG_GETARG_FLOAT8(0);
  float8 dec_deg = PG_GETARG_FLOAT8(1);
  ...
  c->center = *spos(ra_deg, dec_deg);
  c->radius = deg2rad(radius);
