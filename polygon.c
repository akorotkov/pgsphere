 ...
 */
static void spherepoly_center(Vector3D *v, const SPOLY *poly) {
  int32 i;
  Vector3D v1, v2;
...
SPOLY *spherepoly_from_array(SPoint *arr, int32 nelem) {
  ...
  if (nelem < 3) {
    ...
    while (i < (nelem - 1)) {
      ...
      if (spoint_eq(&arr[i], &arr[i + 1])) {
        if (i < (nelem - 2)) {
          memmove((void *)&arr[i + 1], (void *)&arr[i + 2],
    ...
    for (i = 0; i < nelem; i++) {
      ...
      if (FPeq(scheck, PI)) {
        elog(ERROR, "spherepoly_from_array: a polygon segment length must be "
                    "not equal 180 degrees.");
        return NULL;
      }
  ...
  if (!spherepoly_check(poly)) {
    elog(ERROR,
         "spherepoly_from_array: a line segment overlaps or polygon too large");
    pfree(poly);
    return NULL;
  }
 ...
 */
static void euler_spoly_trans(SPOLY *out, const SPOLY *in, const SEuler *se) {
  int32 i;
...
Datum spheretrans_poly_inverse(PG_FUNCTION_ARGS) {
  ...
  ret = DirectFunctionCall2(spheretrans_poly, sp, PointerGetDatum(&tmp));
  PG_RETURN_DATUM(ret);
}
...
Datum spherepoly_add_point(PG_FUNCTION_ARGS) {
  ...
  if (FPeq(spoint_dist(p, &poly->p[poly->npts - 1]), PI)) {
    elog(NOTICE,
         "spoly(spoint): Skip point, distance of previous point is 180deg");
  }
...
Datum spherepoly_add_points_finalize(PG_FUNCTION_ARGS) {
  ...
  if (FPeq(spoint_dist(&poly->p[0], &poly->p[poly->npts - 1]), PI)) {
    elog(NOTICE, "spoly(spoint): Cannot close polygon. Distance between first "
                 "and last point is 180deg");
    pfree(poly);
    PG_RETURN_NULL();
  }
