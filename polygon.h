...
typedef struct
{
	...
	/* !<count of points */
	SPoint		p[1];
	/* !<variable length array of SPoints */
} SPOLY;
...
#define PG_GETARG_SPOLY( arg ) ( ( SPOLY  * ) DatumGetPointer( PG_DETOAST_DATUM( PG_GETARG_DATUM ( arg ) ) ) )
 ...
 /*
  * ! \brief Checks, whether two polygons are equal \param p1 pointer to first
  * polygon \param p2 pointer to second polygon \param dir true, if check with
  * reverse polygon p2 \return true, if polygons are equal
  */
bool		spoly_eq(const SPOLY *p1, const SPOLY *p2, bool dir);

 /*
  * ! \brief Returns the i-th line segment of a polygon \param sl	pointer to
...
SLine	   *spoly_segment(SLine *sl, const SPOLY *poly, int32 i);
  ...
  * \param pg pointer to polygon \return true if polygon contains point
  */
bool		spoly_contains_point(const SPOLY *pg, const SPoint *sp);
  ...
  * \link PGS_LINE_POLY_REL int8 value \endlink (\ref PGS_POLY_LINE_REL )
  */
int8		poly_line_pos(const SPOLY *poly, const SLine *line);

 /*
  * ! \brief input of spherical polygon \return polygon datum \note PostgreSQL
  * function
  */
Datum		spherepoly_in(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether two polygons are equal \return bool datum \note
  * PostgreSQL function
  */
Datum		spherepoly_equal(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether two polygons are not equal \return bool datum
  * \note PostgreSQL function
  */
Datum		spherepoly_equal_neg(PG_FUNCTION_ARGS);

 /*
  * ! \brief Circumstance of a polygon \return circumference in radians
  * (float8 datum) \note PostgreSQL function
  */
Datum		spherepoly_circ(PG_FUNCTION_ARGS);

 /*
  * ! \brief Count of points (edges) of polygon \return int8 datum \note
  * PostgreSQL function
  */
Datum		spherepoly_npts(PG_FUNCTION_ARGS);

 /*
  * ! \brief Returns area of a polygon \return float8 datum \note PostgreSQL
  * function
  */
Datum		spherepoly_area(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether polygon contains point \return bool datum \note
  * PostgreSQL function
  */
Datum		spherepoly_cont_point(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether polygon doesn't contain point \return bool datum
  * \note PostgreSQL function
  */
...
Datum		spherepoly_cont_point_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether polygon contains circle \return bool datum \note
  * PostgreSQL function
  */
Datum		spherepoly_cont_circle(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether polygon doesn't contain circle \return bool datum
  * \note PostgreSQL function
  */
...
Datum		spherepoly_cont_circle_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether circle contains polygon \return bool datum \note
  * PostgreSQL function
  */
Datum		spherecircle_cont_poly(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether circle doesn't contain polygon \return bool datum
  * \note PostgreSQL function
  */
...
Datum		spherecircle_cont_poly_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether polygon and circle are overlapping \return bool
  * datum \note PostgreSQL function
  */
Datum		spherepoly_overlap_circle(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether polygon and circle aren't  overlapping \return
  * bool datum \note PostgreSQL function
  */
...
Datum		spherepoly_overlap_circle_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether polygon contains line \return bool datum \note
  * PostgreSQL function
  */
Datum		spherepoly_cont_line(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether polygon doesn't contain line \return bool datum
  * \note PostgreSQL function
  */
...
Datum		spherepoly_cont_line_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether polygon and line are overlapping \return bool
  * datum \note PostgreSQL function
  */
Datum		spherepoly_overlap_line(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether polygon and line aren't overlapping \return bool
  * datum \note PostgreSQL function
  */
...
Datum		spherepoly_overlap_line_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether a polygon contains a other polygon \return bool
  * datum \note PostgreSQL function
  */
Datum		spherepoly_cont_poly(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether a polygon doesn't contain a other polygon \return
  * bool datum \note PostgreSQL function
  */
...
Datum		spherepoly_cont_poly_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether two polygons are overlapping \return bool datum
  * \note PostgreSQL function
  */
Datum		spherepoly_overlap_poly(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether two polygons are not overlapping \return bool
  * datum \note PostgreSQL function
  */
Datum		spherepoly_overlap_poly_neg(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether a polygon contains an ellipse \return bool datum
  * \note PostgreSQL function
  */
Datum		spherepoly_cont_ellipse(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether a polygon doesn't contain an ellipse \return bool
  * datum \note PostgreSQL function
  */
...
Datum		spherepoly_cont_ellipse_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether an ellipse contains polygon \return bool datum
  * \note PostgreSQL function
  */
Datum		sphereellipse_cont_poly(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether an ellipse doesn't contain polygon \return bool
  * datum \note PostgreSQL function
  */
...
Datum		sphereellipse_cont_poly_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Checks, whether polygon and ellipse are overlapping \return bool
  * datum \note PostgreSQL function
  */
Datum		spherepoly_overlap_ellipse(PG_FUNCTION_ARGS);

 /*
  * ! \brief Checks, whether polygon and ellipse aren't overlapping \return
  * bool datum \note PostgreSQL function
  */
...
Datum		spherepoly_overlap_ellipse_com_neg(PG_FUNCTION_ARGS);
 ...
 /*
  * ! \brief Does transform a polygon using Euler transformation \return
  * Polygon datum \note PostgreSQL function
  */
...
Datum		spheretrans_poly_inverse(PG_FUNCTION_ARGS);
 ...
 /*
  * ! State transition function for aggregate function spoly(spoint). Do never
  * call this function outside an aggregate function! \brief Adds a point to
  * polygon \return Polygon datum \note PostgreSQL function
  */
Datum		spherepoly_add_point(PG_FUNCTION_ARGS);

 /*
  * ! \brief Finalize function for adding spoints to polygon \return Polygon
  * datum \note PostgreSQL function
  */
